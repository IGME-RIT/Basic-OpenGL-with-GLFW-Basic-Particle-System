...
void SpriteBatcher::DrawSquare(glm::vec4 destRect, glm::vec4 sourceRect, glm::vec4 color, Texture* texture)
{
...
void SpriteBatcher::DrawRotated(glm::vec4 destRect, glm::vec4 sourceRect, glm::vec4 color, Texture* texture, glm::vec2 axis, float angle)
{
    // Draw doesn't actually have to draw anything.
    // Instead it collects a bunch of vertex information until flush is called, and then draws it all.
    ...
    glm::vec2 vertex[4];
    vertex[0] = -axis;
    vertex[1] = glm::vec2(destRect.z, 0) - axis;
    vertex[2] = glm::vec2(0, destRect.w) - axis;
    vertex[3] = glm::vec2(destRect.z, destRect.w) - axis;
    ...
    for (int i = 0; i < 4; i++)
    {
        vertex[i] = rotation * vertex[i] + glm::vec2(destRect.x, destRect.y);
    }

    // Create vertices for the given draw request and add them to the buffer.
    // A more complex spritebatcher may allow for rotation of vertices, which would also happen here
    m_vertexBuffer.push_back(Vertex2dUVColor(vertex[0], glm::vec2(sourceRect.x, sourceRect.y), color));
    m_vertexBuffer.push_back(Vertex2dUVColor(vertex[1], glm::vec2(sourceRect.z, sourceRect.y), color));
    m_vertexBuffer.push_back(Vertex2dUVColor(vertex[2], glm::vec2(sourceRect.x, sourceRect.w), color));
    m_vertexBuffer.push_back(Vertex2dUVColor(vertex[1], glm::vec2(sourceRect.z, sourceRect.y), color));
    m_vertexBuffer.push_back(Vertex2dUVColor(vertex[2], glm::vec2(sourceRect.x, sourceRect.w), color));
    m_vertexBuffer.push_back(Vertex2dUVColor(vertex[3], glm::vec2(sourceRect.z, sourceRect.w), color));
}
