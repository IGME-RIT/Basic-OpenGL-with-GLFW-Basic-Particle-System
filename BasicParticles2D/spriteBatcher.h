...
public:
    ...
    // Draws at the target rectangle, Rectangle is defined as a bottom left corner, and a size
    void DrawSquare(glm::vec4 destRect, glm::vec4 sourceRect, glm::vec4 color, Texture* texture);
    // Draws sprite at , rotated around the given axis (relative to the position) by the given angle
    void DrawRotated(glm::vec4 destRect, glm::vec4 sourceRect, glm::vec4 color, Texture* texture, glm::vec2 axis, float angle);
    void Flush();
